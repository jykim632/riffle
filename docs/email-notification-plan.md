...
| 항목 | 결정 | 이유 |
|------|------|------|
| 이메일 서비스 | Resend | 무료 3,000건/월, Next.js 연동 간단 |
...
| `src/lib/email/resend.ts` | 신규 |
| `src/lib/email/send-summary-notification.ts` | 신규 |
| `src/actions/notifications.ts` | 신규 |
| `src/actions/summaries.ts` | 수정 |
| `src/components/dashboard/user-menu.tsx` | 수정 |
| `.env.example` | 수정 |
...
- Resend 무료 티어 제한: 월 3,000건, 일 100건
- 프로덕션에서 발신 도메인 인증 필요 (Resend 대시보드에서 DNS 설정)
...
4. 요약본 제출 후 이메일 수신 확인 (Resend 테스트 모드)
