...
**날짜**: 2026-02-08
**커밋**:
- `efc4553` feat: 모바일 반응형 최적화 (Admin + 대시보드 헤더 + 티커)
- `f4e0213` fix: 시장요약 위젯 모바일 2x2 그리드 + 빈 변동값 숨김
...
### 4. 시장요약 위젯 모바일 그리드
- `flex overflow-x-auto` → `grid grid-cols-2` (모바일 2x2 그리드)
- sm 이상: `sm:flex` 한 줄 레이아웃 유지
- 구분선(`|`): `hidden sm:inline` 모바일에서 숨김
- 변동 데이터 없을 때 `─` placeholder 제거 → 아예 비표시
...
- **시장요약 위젯 한줄 vs 그리드**: 지표 4개라 flex 한줄은 모바일에서 가로 스크롤 불가피. 2x2 그리드가 정보 밀도와 가독성 모두 좋음.
...
| 시장요약 grid-cols-2 | 지표 4개 → 2x2가 자연스러움, flex-wrap보다 정돈됨 |
| 빈 변동값 비표시 | ─ placeholder는 정보 아님, 공간만 차지 |
